<template>
  <view class="w-picker-view">
    <picker-view class="d-picker-view" :indicator-style="itemHeight" :value="pickVal" @change="handlerChange">
      ...
      <picker-view-column>
        <view class="w-picker-item" v-for="(item, index) in range.hours" :key="index">{{ item.label }}{{ $t('时') }}</view>
      </picker-view-column>
      <picker-view-column>
        <view class="w-picker-item" v-for="(item, index) in range.minutes" :key="index">{{ item.label }}{{ $t('分') }}</view>
      </picker-view-column>
    </picker-view>
  </view>
</template>
  ...
  methods: {
    ...
    getData(dVal) {
      ...
      for (let i = 0; i < this.expand * 1; i++) {
        ...
        switch (i) {
          case 0:
            label = global.i18n.t('今天')
            break
          case 1:
            label = global.i18n.t('明天')
            break
          case 2:
            label = global.i18n.t('后天')
            break
        }
        dates.push({
          label: label,
