  ...
  <view class="w-picker-view">
    <picker-view class="d-picker-view" :indicator-style="itemHeight" :value="pickVal" @change="handlerChange">
      <picker-view-column class="w-picker-flex2">
        <view class="w-picker-item" v-for="(item, index) in range.fyears" :key="index">{{ item }}{{ $t('年') }}</view>
      </picker-view-column>
      <picker-view-column class="w-picker-flex2">
        <view class="w-picker-item" v-for="(item, index) in range.fmonths" :key="index">{{ item }}{{ $t('月') }}</view>
      </picker-view-column>
      <picker-view-column class="w-picker-flex2">
        <view class="w-picker-item" v-for="(item, index) in range.fdays" :key="index">{{ item }}{{ $t('日') }}</view>
      </picker-view-column>
      <picker-view-column class="w-picker-flex1">
        <view class="w-picker-item">-</view>
      </picker-view-column>
      <picker-view-column class="w-picker-flex2">
        <view class="w-picker-item" v-for="(item, index) in range.tyears" :key="index">{{ item }}{{ $t('年') }}</view>
      </picker-view-column>
      <picker-view-column class="w-picker-flex2">
        <view class="w-picker-item" v-for="(item, index) in range.tmonths" :key="index">{{ item }}{{ $t('月') }}</view>
      </picker-view-column>
      <picker-view-column class="w-picker-flex2">
        <view class="w-picker-item" v-for="(item, index) in range.tdays" :key="index">{{ item }}{{ $t('日') }}</view>
      </picker-view-column>
    </picker-view>
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    initData() {
      ...
      result = full = `${fyear + '-' + fmonth + '-' + fday + this.$t('至') + tyear + '-' + tmonth + '-' + tday}`
      this.range = range
      this.checkObj = obj
      this.$nextTick(() => {
    ...
    handlerChange(e) {
      ...
      result = full = `${fyear + '-' + fmonth + '-' + fday + global.i18n.t('至') + tyear + '-' + tmonth + '-' + tday}`
      obj = {
        fyear,
        fmonth,
        fday,
