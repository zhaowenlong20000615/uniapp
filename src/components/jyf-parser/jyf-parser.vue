    ...
    setContent(html, append) {
      ...
      if (append)
        this.$refs.web.evalJs(
          "var b=document.createElement('div');b.innerHTML='" + html.replace(/'/g, "\\'") + "';document.getElementById('parser').appendChild(b)"
        )
      else {
        html =
      ...
      var fill = (target) => {
        ...
        if (this.domain && src) {
          if (src[0] == '/') {
            if (src[1] == '/') target.src = (this.domain.includes('://') ? this.domain.split('://')[0] : '') + ':' + src
      ...
      for (let video, i = 0; (video = videos[i++]); ) {
        ...
        video.onplay = function () {
          if (_ts.autopause) for (let item, i = 0; (item = _ts.videoContexts[i++]); ) if (item != this) item.pause()
        }
      }
      // 音频处理
